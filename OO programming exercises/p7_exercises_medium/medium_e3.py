'''Exercise 3 Number Guesser Part 2
...
class GuessingGame:
    ...
    def play(self):
        ...
        oportunities = int(math.log2(self.high - self.low + 1)) + 1

        while oportunities > 0:
            print(f"You have {oportunities} guesses remaining.")
            while True:
                my_str_num = input(f"Enter a number between {self.low} and {self.high}: ")
                if my_str_num.isdigit():
                    my_num = int(my_str_num)
                    if self.low <= my_num <= self.high:
                        break
                print("Invalid guess")
